...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f (State s) = State (\r -> let (a, s') = s r in (f a, s'))
...
findM fa (h:.t) = (\cond -> if cond then pure (Full h) else findM fa t) =<< fa h
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat la = let p x = (State $ \s -> (S.member x s, S.insert x s)) in eval (findM p la) S.empty
...
distinct ::
  ...
  List a
  -> List a
distinct la = let p x = (State $ \s -> (S.notMember x s, S.insert x s)) in eval (filtering p la) S.empty
